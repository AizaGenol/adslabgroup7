...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($name_error_err) && empty($classification_error) && empty($description_error) && empty($equipments_error) && empty($rules_error) && empty($history_error)) {
          $sql = "INSERT INTO `users` (`name`, `classification`, `description`, `equipments`, `rules`, `history`) VALUES ('$name', '$classification', '$description', '$equipments', '$rules', '$history')";
                    ...
                    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
                        ...
                        <div class="form-group <?php echo (!empty($classification_error)) ? 'has-error' : ''; ?>">
                            <label>Classification:</label>
                                <select name="classification" id="classification" style="padding: 8px 512px; border-radius: 5px;">
